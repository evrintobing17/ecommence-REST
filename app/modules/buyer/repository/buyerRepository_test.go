...
func (s *Suite) TestCreate() {
	...
	repository := NewBuyerRepository(db)

	entities := models.Buyer{
		ID:       1,
		Name:     "TEST",
		Email:    "TEST",
		Password: "TEST",
...
func (s *Suite) TestUpdate() {
	...
	s.T().Run("success", func(t *testing.T) {
		query := `SELECT * FROM "ecommerce"."buyer" WHERE (id=$1) ORDER BY "ecommerce"."buyer"."id" ASC LIMIT 1`
...
func (s *Suite) TestGetByName() {
	...
	s.T().Run("error update", func(t *testing.T) {
		query := `SELECT * FROM "ecommerce"."buyer" WHERE (name=$1) ORDER BY "ecommerce"."buyer"."id" ASC LIMIT 1`
		queryRegex := regexp.QuoteMeta(query)
		mock.ExpectQuery(queryRegex).WithArgs().WillReturnRows(sqlmock.NewRows([]string{"id", "name"}).AddRow(entities.ID, entities.Name))
...
func (s *Suite) TestExistEmail() {
	...
	s.T().Run("error update", func(t *testing.T) {
		query := `SELECT * FROM "ecommerce"."buyer" WHERE (email=$1) ORDER BY "ecommerce"."buyer"."id" ASC LIMIT 1`
		queryRegex := regexp.QuoteMeta(query)
		mock.ExpectQuery(queryRegex).WithArgs().WillReturnRows(sqlmock.NewRows([]string{"id", "name"}).AddRow(entities.ID, entities.Name))
