...
func NewBuyerUsecase(repo buyer.BuyerRepository) buyer.BuyerUsecase {
	return &UC{
		repo: repo,
	}
}

func (uc *UC) Login(email, password string) (user *models.Buyer, token string, err error) {
...
func (uc *UC) Register(username, email, password, address string) (driver *models.Buyer, err error) {
	//Cast ReqRegister to model.User

	userModel := models.Buyer{
		Name:     username,
		Email:    email,
		Password: password,
		Address:  address,
	}
	errUser := uc.isBuyerExist(userModel)
	if errUser != nil {
...
func (uc *UC) Update(updateData map[string]interface{}) (*models.Buyer, error) {
...
func (uc *UC) isBuyerExist(user models.Buyer) error {
